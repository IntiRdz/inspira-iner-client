...
import { BacteriumIcon } from '../components/icons/BacteriumIcon';  
...
const PacienteHosp = ({ paciente, contador }) => {
    ...
    return ( 
        <tr>
            ...
            <td className="border px-1 py-1">{paciente.expediente}</td>
            ...
            <td className="border px-1 py-1">{paciente.pac_apellido_paterno}</td>
            <td className="border px-1 py-1">{paciente.pac_apellido_materno}</td>
            <td className="border px-1 py-1">{paciente.pac_nombre}</td>
            <td className="border px-1 py-1">{calcularEdad(paciente.pac_FN)}</td>
            <td className="border border-l-4 border-l-sky-700 px-1 py-1">{paciente.pac_genero}</td>
            <td className="border px-1 py-1">{paciente.pac_dispositivo_o2}</td>
            <td className="border px-1 py-1">{paciente.pac_hemodialisis ? 'Sí' : 'No'}</td>
            <td className="border px-1 py-1">{paciente.caracteristicas_especiales}</td>
            ...
            <td className="border px-1 py-1">{paciente.diagnostico}</td>
            <td className="border border-l-4 border-l-sky-700 px-1 py-1">{paciente.fecha_ingreso? format(new Date(paciente.fecha_ingreso), 'dd-MM-yy') : ''}</td>
            <td className="border px-1 py-1">{calcularDias(paciente.fecha_ingreso)}</td>
            <td className={fechaPreAltaClasses}>{paciente.fecha_prealta? format(new Date(paciente.fecha_prealta), 'dd-MM-yy') : ''}</td>
            <td className="border px-1 py-1">
                <button 
