...
import { OBTENER_CAMAS_NO_0 } from '../../graphql/queries'; 

const timeZone = 'America/Mexico_City'; // Define la zona horaria

export default function ClinicoAllNo0Simple (){

  const router = useRouter();

  const { data, loading, error } = useQuery(OBTENER_CAMAS_NO_0);
          ...
          <table className="table-auto shadow-md mt-2 w-full w-lg ">
            <thead className="bg-gray-800 ">
              <tr className="text-white">
                  ...
                  <th className="w-1/13 px-1 py-1">Cama</th>
                  <th className="w-1/13 px-1 py-1">Prioridad</th>
                  ...
                  <th className="w-1/13 px-1 py-1">DAN</th>
                  ...
                  <th className="w-1/5 px-1 py-1">Dx</th>
                  <th className="w-1/10 px-1 py-1">Microorganismos</th>
              </tr>
            </thead>
            <tbody>
            ...
            return (
              <tr key={cama.id} className={`${cama.cama_lado === 'Arriba' ? 'border-t-2 border-t-sky-700' : cama.cama_lado === 'Bajo' ? 'border-b-2 border-b-sky-700' : ''} ${!cama.cama_ocupada ? 'bg-gray-200' : ''}`}>
                ...
                <td className={`border px-1 text-center border-r-2 border-r-sky-700 ${cama.cama_dan ? 'bg-red-200' : ''}`}>{cama.cama_dan ? <SoapIcon width="2rem" height="2rem" style={{ display: 'inline-block' }} /> : ''}</td>
