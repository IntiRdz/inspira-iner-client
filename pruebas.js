...
const DiagnosticosPaciente = ({ obtenerPaciente: { id } }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [diagnosticoActual, setDiagnosticoActual] = useState(null);

    const { data, loading, error } = useQuery(OBTENER_ULTIMA_ADMISION_PACIENTE, { variables: { id } });
    ...
    const handleEditClick = diagnosticoId => {
        setDiagnosticoActual(ultimaAdmision.diagnostico.find(d => d.id === diagnosticoId));
        setIsModalOpen(true);
    };

    const handleInputChange = e => {
        setDiagnosticoActual(prevState => ({ ...prevState, [e.target.name]: e.target.value }));
    };
